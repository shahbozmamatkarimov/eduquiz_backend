...
export class BotService {
    ...
    try {
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          Markup.button.webApp('Open Mini App', 'https://uz.online'),
        ]),
      );
      if (!user) {
        ...
        await ctx.reply(
          "Bu bot orqali EduQuiz dasturi orqali ro'yhatga o'tilgan",
          {
            parse_mode: 'HTML',
            ...Markup.keyboard([
              ["Parolni o'zgaritish", "Telefon raqamni o'zgartirish"],
  ...
  async setPassword(@Ctx() ctx: Context) {
    ...
    if (!user?.user_id) {
      ...
      const url = `https://www.eduquiz.uz/login?token=${bot_user.token}`;
      await ctx.reply(`[EduQuiz online saytiga kirish uchun shu yerga bosing](${url})`, { parse_mode: 'MarkdownV2' });
    } else {
